...
class PyInstArchive:
    ...
    def extractFiles(self, extraction_id):
        print('[+] Beginning extraction...')

        baseDir = os.path.join("pending_decompilation", extraction_id)
        ...
        for entry in self.tocList:
            ...
            if entry.cmprsFlag == 1:
                try:
                    ...
                    print(f'[!] Failed to decompress {entry.name}')
                    continue

            if entry.typeCmprsData in [b'd', b'o']:
                continue

            safe_name = entry.name.replace('..', '__')
            file_path = os.path.join(baseDir, safe_name)

            os.makedirs(os.path.dirname(file_path), exist_ok=True)

            if entry.typeCmprsData == b's':
                if self.pycMagic == b'\0' * 4:
                    self.barePycList.append(file_path + '.pyc')

                self._writePyc(file_path + '.pyc', data)
                ...
                if data[2:4] == b'\r\n':
                    ...
                    self._writeRawData(file_path + '.pyc', data)
                else:
                    if self.pycMagic == b'\0' * 4:
                        self.barePycList.append(file_path + '.pyc')
                ...
                self._writeRawData(file_path, data)

                if entry.typeCmprsData in [b'z', b'Z']:
                    self._extractPyz(file_path)
...
def main():
    if len(sys.argv) < 2:
        print('[!] Usage: pyinstxtractor.py <id>')
        return
    ...
    if not os.path.exists(base_dir):
        print(f"[!] Error: ID folder not found: {base_dir}")
        return
    ...
    if len(exe_files) == 0:
        print("[!] Error: No .exe file found in the folder")
        return

    if len(exe_files) > 1:
        print("[!] Error: Multiple .exe files found, ambiguous")
        return

    exe_path = os.path.join(base_dir, exe_files[0])

    print(f"[+] Using file: {exe_path}")
